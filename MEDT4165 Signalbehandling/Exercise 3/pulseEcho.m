...
%% pulse parameters
f0=2.5e6;
Tp=3e-6;%pulse length %1/f0*Nperiods (1/f0*8)
lambda=c/f0;
...
Z=ones(size(z))*Z1;
%Z((1+round(z1/dz):1+round(z2/dz)));

Gn=normrnd(1.66,0.02,[size(Z),1]);

count=1;
for i=(1+round(z1/dz):1+round(z2/dz))%medium with fat/muscle
    Z(i)=Gn(count);
    count=count+1;
end
%
%for i=(1+round(z1/dz):1+round(z2/dz))
%   Z(i)=1.66+0.02*sin(2*pi*(1/0.385e-3)*z(i));
%end 
